<template>
    <div class="m-6 p-6 border-purple-500 border-2 rounded-xl h-fit">
        <div class="mb-3 flex justify-between"><span class="text-2xl">Level Progression</span> <span class="text-lg">({{ gurudKanji }} / {{ kanjiGurudToLevel }}) Kanji Guru'd to Level</span></div>
        <div id="level-progress-bar" class="flex flex-row justify-between px-1 mb-3">
            <div v-for="index in kanjiGurudToLevel" :key="index"  class="border h-6 w-full mx-0.5" :style="{ backgroundColor: (gurudKanji >= index) ? 'limegreen' : 'gray' }"></div>
        </div>
        ...
        <div class="mb-1"><button @click="toggleVocabulary" class="text-xl border border-purple-500 bg-purple-500 text-white hover:bg-purple-600">Vocabulary ({{ gurudVocabulary }}/{{ vocabulary.length }})</button></div>
        <hr class="mb-3" />
        <div class="mb-6 flex flex-row flex-wrap">
